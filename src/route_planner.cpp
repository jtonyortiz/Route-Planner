...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    ...
    start = &m_Model.FindClosestNode(start_x, start_y);

    start_node = start;

    RouteModel::Node* end = new RouteModel::Node;

    end = &m_Model.FindClosestNode(end_x, end_y);
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) 
{
    ...
    // Use FindNeighbors() Method of the current node to populate current node:
    current_node->FindNeighbors();

    //For-loop: iterate through current_node.neighbors, 
    //set: 
    //parent, h_value, g_value
...
void RoutePlanner::AStarSearch() 
{
    ...
    while(!open_list.empty())
    {
        ...
        //If the current nodes distance to the end node is 0
        if(current_node->distance(*end_node) == 0)
        {
            //Construct model path from the current node:
            m_Model.path = ConstructFinalPath(current_node);
            break;
        }
        else
        {
            //Add all of the neighbors of the current node.
